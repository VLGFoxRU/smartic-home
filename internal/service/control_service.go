...
func (s *ControlService) SendCommand(ctx context.Context, deviceID, command string, params map[string]interface{}, userID, role string) error {
	...
	// 8. Обновление кэша состояния (оптимистично, реальное состояние придёт от устройства)
	newState := map[string]interface{}{"lastCommand": command, "params": params, "status": "pending"}
