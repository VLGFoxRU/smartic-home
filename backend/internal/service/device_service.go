...
func (s *DeviceService) CreateDevice(ctx context.Context, name, devType, roomID string) (*domain.Device, error) {
    ...
    // Записываем начальное состояние в кэш
	var initialState string
	switch devType {
	case "light", "power_switch":
		initialState = `{"power":"off"}`
	default:
		initialState = `{}`
	}
	_ = s.cache.Set(ctx, "device:"+device.ID()+":state", initialState, 0) // без TTL, пока не переопределим
